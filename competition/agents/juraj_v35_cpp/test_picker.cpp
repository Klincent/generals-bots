...
int main(){
 ...
 // A real general emergency may pause other work; once it clears, the same picker resumes.
	 {setenv("V35_EDGE_PICKER_THRESHOLD","4",1);Agent a(0,21,21);auto o=picker_board();auto first=a.decide(o);apply_picker(o,first);int pc=a.edge_picker_cell();int enemy=219;o.owner[enemy]=2;o.army[enemy]=8;o.army[218]=12;recount(o);auto emergency=a.decide(o);assert(emergency.kind==0);assert(psrc(emergency)!=pc);assert(a.edge_picker_active());assert(a.edge_picker_cell()==pc);apply_picker(o,emergency);o.owner[enemy]=1;o.army[enemy]=1;recount(o);for(int i=0;i<80&&a.edge_picker_completions()==0;++i){auto z=a.decide(o);apply_picker(o,z);}assert(a.edge_picker_completions()==1);assert(a.edge_picker_aborts()==0);assert(a.edge_picker_paused()>0);}
