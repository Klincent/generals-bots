...
int main(){
 ...
 // 7-13 exact directed-edge history: return is legal, recently repeated directed edge is not.
 Packet p;p.target=8;p.event_version=1;
 p.cell=2;p.path={1,2};assert(route_allowed(p,1,2,2,1,Reason::NONE)); // A-B-A allowed
 p.cell=1;p.path={1,2,1};assert(!route_allowed(p,2,2,2,1,Reason::NONE)); // A-B-A-B blocked
 p.cell=3;p.path={1,2,3};assert(route_allowed(p,2,2,2,1,Reason::NONE)); // A-B-C-B allowed
 p.cell=2;p.path={1,2,3,2};assert(!route_allowed(p,3,2,2,1,Reason::NONE)); // then B-C blocked
 p.cell=1;p.path={1,2,3,4,1};assert(!route_allowed(p,2,2,2,1,Reason::NONE)); // A-B still within four transitions
 p.cell=6;p.path={2,3,4,5,6};assert(route_allowed(p,1,2,2,1,Reason::NONE)); // old A-B expired
 assert(route_allowed(p,2,3,2,2,Reason::GENERAL_EMERGENCY));
 // 14 contact creates war surplus and 15 FULL remains default.
 auto b=budget(200,20,15,20,true,.6);assert(b.war>0&&b.front==b.war);assert(!rear.split);
 // 16 swarm response and 17 doomstack response avoid chase/head-on policy.
 ...
 // 19 castle builder chooses flank response.
 ...
 // 20 turtle and 21 multi-front adaptations.
 ...
 // 22 low evidence cannot flip confidence on one observation.
 ...
 // 23 flow belief back-projection moves the top candidate upstream.
 ...
 // 24 scheduler output is deterministic and graph moves are legal-adjacent.
