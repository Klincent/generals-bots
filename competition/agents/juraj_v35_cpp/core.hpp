...
inline CastleChoice plan_castles(const Graph&g,int general){CastleChoice best;int n=g.h*g.w;auto eligible=[&](int x,int deadline){return x!=general&&g.passable[x]&&g.dist[general][x]>=5&&g.dist[general][x]<INF&&g.dist[general][x]+35<=deadline&&g.degree(x)>1;};for(int a=0;a<n;++a)if(eligible(a,150))for(int b=0;b<n;++b)if(b!=a&&eligible(b,250)){int ca=castle_cost(g,general,a,{}),cb=castle_cost(g,general,b,{general,a});auto key=std::tuple(ca+cb,std::max(g.dist[general][a],g.dist[general][b]),-(g.degree(a)+g.degree(b)),a,b);auto old=std::tuple(best.total,best.c1<0?INF:std::max(g.dist[general][best.c1],g.dist[general][best.c2]),best.c1<0?0:-(g.degree(best.c1)+g.degree(best.c2)),best.c1,best.c2);if(key<old)best={a,b,-1,ca,cb,ca+cb};}for(int x=0;x<n;++x)if(eligible(x,350)&&x!=best.c1&&x!=best.c2){best.c3=x;break;}return best;}
