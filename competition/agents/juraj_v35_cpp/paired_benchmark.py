...
for seed in range(a.start,a.start+a.seeds):
 ...
 for swap in (False,True):
  ...
  audit=a.output/f'audit-{seed}-{candidate_seat}.json'; env=os.environ.copy();env['JURAJ_RNG_SEED']=str(rng_seed);env['JURAJ_V35_TRACE']='1';env['PYTHONPATH']=str(Path.cwd())
...
rng=random.Random(0x35);boots=[statistics.mean(rng.choice(paired) for _ in paired) for _ in range(10000)] if paired else [0]
boots.sort()
...
summary={'seed_start':a.start,'seed_end':a.start+a.seeds-1,'maps':a.seeds,'games':len(games),'W':W,'D':D,'L':L,'score':score,'seat_score':seat,'paired_score':statistics.mean(paired),'paired_ci95':[quantile(boots,.025),quantile(boots,.975)],'errors':sum('error'in g for g in games),'illegal_actions':sum(g.get('candidate_illegal',0) for g in games),'turns_mean':statistics.mean([g['turns'] for g in valid]) if valid else 0,'decision_roundtrip_ms':{'p50':quantile(sorted(timing),.5) if timing else 0,'p95':quantile(sorted(timing),.95) if timing else 0,'p99':quantile(sorted(timing),.99) if timing else 0,'max':max((g['candidate_timing']['max'] for g in valid),default=0)}}
