"""Synthetic filrichz-style stress opponent.

Expand broadly, then repeatedly pull even small rear stacks through owned land
into one locked carrier.  It deliberately waits for a genuinely large mass
instead of launching the 20-50 army packets that made rusher-v1 too weak.
"""
...
class Agent:
    ...
    def expand(self,o):
        ...
        for r,c in self.owned(o):
            ...
            for d,(dr,dc) in enumerate(DIRS):
                ...
                score=a+(50000 if typ==4 else 0)+(10000 if owner==2 else 3000 if typ not in (0,5) else 1200)
    ...
    def trace(self,o,phase,carrier,feeders=0):
        ...
        sys.stderr.write(f"[doomstack_rusher] turn={o.turn} phase={phase} carrier={ca} share={ca/max(1,o.my_army):.3f} my_army={o.my_army} my_land={o.my_land} feeders={feeders}\n");sys.stderr.flush()

    def act(self,o):
        for p in self.owned(o):
            if o.type_grid[p[0]][p[1]]==4:self.own_general=p;break

        # Build economy first.  Unlike v1, do not switch to harvest merely
        # because 28 cells were reached very early.
        if o.turn<125:
        ...
        # Require a real doomstack.  The relative threshold scales to smaller
        # boards; the absolute target is deliberately much higher than v1.
        ready=(ca>=90 and ca>=int(.48*max(1,o.my_army)))
        forced=(o.turn>=500 and ca>=70)
        ...
        if not self.attack:
            ...
            for p,d in dist.items():
                ...
                # small edge/rear stacks matter; larger packets win ties.
            ...
            # Keep growing/connecting territory if possible.  Otherwise WAIT
            # for the next production tick and harvest again; never launch a
            # small fallback packet just because the current feeder list is dry.
            m=self.expand(o)
