...
def fresh_worktree(prefix: str, ref: str) -> Path:
...
def patch_one(s: str, old: str, new: str) -> str:
    if s.count(old) != 1:
        raise RuntimeError(f'anchor count {s.count(old)} for {old!r}')
...
def build_source(src: str, label: str) -> Path:
    d = fresh_worktree(f'p9-{label}-', START_REF)
    p = d / AGENT / 'main.cpp'
    p.write_text(src)
...
def bench(candidate: Path, baseline: Path, seed: int, seeds: int, out: Path):
    if out.exists():
        shutil.rmtree(out)
    run([
        'python', str(AGENT / 'paired_benchmark.py'),
        '--candidate', str(candidate), '--baseline', str(baseline),
        '--start', str(seed), '--seeds', str(seeds), '--output', str(out)
    ])
...
def losses(out: Path):
    ...
    for r in rows:
        ...
        if val in ('loss', 'l', 'baseline', '1') or r.get('candidate_score') == 0:
            bad.append({
                'seed': r.get('seed'),
                'seat': r.get('candidate_seat'),
                'turns': r.get('turns'),
            })
    return bad[:20]


def beats_control(summary: dict) -> bool:
    return (
        summary.get('errors', 0) == 0
        and summary.get('illegal_actions', 0) == 0
        and summary.get('score', 0) > 0.5
    )
...
def main():
    ...
    if state_path.exists():
        ...
        state = {'outer_done': 0, 'history': [], 'best_source': None, 'best_score': 0.0}

    start_src = run(['git', 'show', f'{START_REF}:{AGENT}/main.cpp'], capture=True)
    src = state.get('best_source') or start_src
    current_best = build_source(src, f'outer{outer}-start')
    ...
    for i in range(1, 11):
        ...
        # 1 Build + 2 test against the previous best + 3 identify losses.
        control_out = Path(f'/tmp/p9-o{outer}-i{i}-control')
        control_summary = bench(control, current_best, seed, 4, control_out)
        loss_rows = losses(control_out)

        # 4 Defense repair and retest against the version it is trying to replace.
        dsrc, dname = mutate(control_src, i, 'defense')
        defense = build_source(dsrc, f'o{outer}i{i}-def')
        defense_out = Path(f'/tmp/p9-o{outer}-i{i}-def')
        dsum = bench(defense, control, seed + 4, 4, defense_out)
        if beats_control(dsum):
            chosen_src, chosen, defense_kept = dsrc, defense, True
        else:
            chosen_src, chosen, defense_kept = control_src, control, False

        # 5 Improve attack, rebuild, and test against whichever version won above.
        asrc, aname = mutate(chosen_src, i, 'attack')
        attack = build_source(asrc, f'o{outer}i{i}-atk')
        attack_out = Path(f'/tmp/p9-o{outer}-i{i}-atk')
        asum = bench(attack, chosen, seed + 8, 4, attack_out)
        if beats_control(asum):
            src, decision = asrc, 'attack'
        else:
            src = chosen_src
            decision = 'defense' if defense_kept else 'control'

        current_best = build_source(src, f'o{outer}i{i}-best')
        state['history'].append({
            'outer': outer,
            'inner': i,
            'losses': loss_rows,
            'control': control_summary,
            'defense_mutation': dname,
            'defense': dsum,
            'attack_mutation': aname,
            'attack': asum,
            'decision': decision,
        })
        # Local checkpoint after every inner iteration. The workflow commits after each outer block.
        state['best_source'] = src
        state['inner_done'] = i
        state_path.write_text(json.dumps(state, indent=2, sort_keys=True) + '\n')
        print(json.dumps({'outer': outer, 'inner': i, 'decision': decision}, sort_keys=True), flush=True)

    # Regression after every block of ten.
    regression = []
    total = {'W': 0, 'D': 0, 'L': 0, 'games': 0, 'errors': 0, 'illegal_actions': 0}
    candidate = build_source(src, f'outer{outer}-reg')
    for j, ref in enumerate(OPPONENTS):
        od = fresh_worktree(f'p9-opp-{outer}-{j}-', ref)
        run(['bash', str(od / AGENT / 'build.sh')], cwd=od)
        opp = od / 'opp.sh'
        opp.write_text('#!/usr/bin/env bash\n' + f'exec "{od / AGENT / "run.sh"}" "$@"\n')
        opp.chmod(0o755)
        sm = bench(candidate, opp, 65000 + outer * 100 + j * 10, 4, Path(f'/tmp/p9-reg-{outer}-{j}'))
        regression.append({'opponent': ref, **sm})
        for k in total:
            total[k] += sm.get(k, 0)

    total['raw_win_rate'] = total['W'] / total['games'] if total['games'] else 0.0
    total['score'] = (total['W'] + 0.5 * total['D']) / total['games'] if total['games'] else 0.0
    state['outer_done'] = outer
    state['inner_done'] = 10
    state['best_source'] = src
    state['best_score'] = total['score']
    state['last_regression'] = {'opponents': regression, 'aggregate': total}
    state_path.write_text(json.dumps(state, indent=2, sort_keys=True) + '\n')
    Path('/tmp/picker9_evolution_result.json').write_text(
        json.dumps(state['last_regression'], indent=2, sort_keys=True) + '\n'
    )
    print(json.dumps({'outer': outer, 'aggregate': total}, sort_keys=True), flush=True)
