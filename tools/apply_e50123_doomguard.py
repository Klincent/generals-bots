...
replace_once(
    'owned_castle_history_.assign(n_,0);for(int x=0;x<n_;++x)if(o.owner[x]==1&&o.type[x]==3)owned_castle_history_[x]=1;std::fprintf',
    'owned_castle_history_.assign(n_,0);for(int x=0;x<n_;++x)if(o.owner[x]==1&&o.type[x]==3)owned_castle_history_[x]=1;if(const char*e=std::getenv("V35_DOOMGUARD_ENABLED"))doomguard_enabled_=std::atoi(e)!=0;std::fprintf',
    'doomguard env gate',
)
...
replace_once(
    'if(a.kind==0){if(q.reason==Reason::GENERAL_EMERGENCY&&q.role==PacketRole::REACTION)',
    'if(a.kind==0){if(doomguard_active_&&q.reason==Reason::GENERAL_EMERGENCY&&q.role==PacketRole::GENERAL_DEFENSE&&q.target==doom_muster_)++doom_moves_;if(q.reason==Reason::GENERAL_EMERGENCY&&q.role==PacketRole::REACTION)',
